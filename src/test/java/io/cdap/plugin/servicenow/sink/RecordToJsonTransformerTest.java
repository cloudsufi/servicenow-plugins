...
public class RecordToJsonTransformerTest {
  ...
  public void testTransformWithValue() {
    ...
    values[0] = 12;
    Mockito.when(record.get("bytes")).thenReturn(values);
    Mockito.when(record.get("updatedPrice")).thenReturn(45.8000);
    ...
    Assert.assertEquals("1", recordToJsonTransformer.transform(record).get("id").getAsString());
    Assert.assertEquals("1970-01-18T11:01:24.583259Z[UTC]", recordToJsonTransformer.transform(record).
