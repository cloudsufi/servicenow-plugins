...
Feature: ServiceNow Sink - Run time scenarios (macro)
  ...
  @TS-SN-RNTM-SINK-MACRO-05 @BQ_SINK_TEST @SN_SOURCE_CONFIG @SN_RECEIVING_SLIP_LINE
  Scenario: Verify pipeline failure message in logs when user provides invalid Table Name with Macros
    And Select data pipeline type as: "Data Pipeline - Batch"
    And Select plugin: "BigQuery" from the plugins list as: "Source"
    And Navigate to the properties page of plugin: "BigQuery"
    And Fill Reference Name
  ...
  @TS-SN-RNTM-SINK-MACRO-06 @BQ_SINK_TEST @SN_SOURCE_CONFIG @SN_RECEIVING_SLIP_LINE
