...
public class ServiceNowSinkConfig extends ServiceNowBaseConfig {
  ...
  @Description("No. of requests that will be sent to ServiceNow Batch API as a payload. Default value is 200. Rest " +
    ...
    "in a batch. To use a bigger page size set it to a higher value. ")
   ...
   * @param pageSize The number of records per batch
   */
  public ServiceNowSinkConfig(String referenceName, String clientId, String clientSecret, String restApiEndpoint,
                              ...
                              String operation, Long pageSize) {
  ...
  void validatePageSize(FailureCollector collector) {
    if (containsMacro(ServiceNowConstants.PROPERTY_PAGE_SIZE)) {
      return;
    }

    if (Objects.isNull(pageSize)) {
      collector.addFailure("Page Size must be specified.", null)
        .withConfigProperty(ServiceNowConstants.PROPERTY_PAGE_SIZE);
    }

    if (!containsMacro(ServiceNowConstants.PROPERTY_PAGE_SIZE) && (getPageSize() > 500 || getPageSize() < 200)) {
      collector.addFailure("Page Size must not be greater than 500 or less than 200.",
                           null).withConfigProperty(ServiceNowConstants.PROPERTY_PAGE_SIZE);
    }
  }
    ...
    for (Schema.Field providedField : Objects.requireNonNull(providedSchema.getFields())) {
      ...
      if (actualField == null) {
        throw new IllegalArgumentException(
          String.format("Field '%s' does not exist in ServiceNow", providedField.getName()));
      }
